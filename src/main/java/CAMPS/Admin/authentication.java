...
@WebServlet(name = "authentication_controller",description = "authentication_controller",urlPatterns = {"/checkLogin"} )
...
public class authentication extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (gkey != null) {
                db.read("SELECT IFNULL(sm.department_id,sam.branch_id) db_id,um.user_id,um.roles,um.user_name,um.status,NOW() lastlogin,CASE WHEN um.staff_id IS NOT NULL THEN 'Staff' WHEN stu.student_id IS NOT NULL THEN 'Student' WHEN mr.student_id IS NOT NULL THEN 'Parent' END ss_det ,IF(um.staff_id IS NOT NULL,sm.staff_id,stu.student_id) staff_id,IF(um.staff_id IS NOT NULL,TRIM(CONCAT(IFNULL(sm.first_name,''),' ',IFNULL(sm.middle_name,''),' ',IFNULL(sm.last_name,''),' ')),TRIM(CONCAT(IFNULL(stu.first_name,''),' ',IFNULL(stu.middle_name,''),' ',IFNULL(stu.last_name,''),' '))) staff_name,IF(um.staff_id IS NOT NULL,sm.institute_email_id,stu.institute_email_id)institute_email_id FROM admin.user_master um LEFT JOIN camps.staff_master sm ON um.staff_id=sm.staff_id LEFT JOIN camps.student_master stu ON stu.student_id=um.student_id LEFT JOIN camps.master_relation mr ON mr.student_id=um.student_id LEFT JOIN student_admission_master sam ON sam.student_id=um.student_id WHERE stu.institute_email_id='" + gkey + "' OR sm.institute_email_id='" + gkey + "' OR mr.email_id='" + gkey + "' AND um.status>0 ");
                if (db.rs.next()) {
                    db.rs.beforeFirst();
                } else {
                    db.read("SELECT sam.branch_id db_id,CONCAT('S',sam.student_id) user_id,30 roles,stu.first_name user_name,1 status,NOW() lastlogin,'Student' ss_det,stu.student_id staff_id,CONCAT(IFNULL(stu.first_name,''),' ',IFNULL(stu.middle_name,''),' ',IFNULL(stu.last_name,'')) staff_name,stu.institute_email_id FROM camps.student_master stu INNER JOIN camps.student_admission_master sam ON stu.student_id=sam.student_id WHERE stu.institute_email_id='" + gkey + "' AND sam.student_status LIKE 'conti%'");
                }
            } else {
                db.read("SELECT IFNULL(sm.department_id,sam.branch_id) db_id,um.user_id,um.roles,um.user_name,um.status,now() lastlogin,IF(um.staff_id IS NOT NULL,'Staff','Student') ss_det ,IF(um.staff_id IS NOT NULL,sm.staff_id,stu.student_id) staff_id,IF(um.staff_id IS NOT NULL,CONCAT(IFNULL(sm.first_name,''),' ',IFNULL(sm.middle_name,''),' ',IFNULL(sm.last_name,'')),CONCAT(IFNULL(stu.first_name,''),' ',IFNULL(stu.middle_name,''),' ',IFNULL(stu.last_name,''))) staff_name,IF(um.staff_id IS NOT NULL,sm.institute_email_id,stu.institute_email_id)institute_email_id FROM admin.user_master um LEFT JOIN camps.staff_master sm ON um.staff_id=sm.staff_id LEFT JOIN camps.student_master stu ON stu.student_id=um.student_id LEFT JOIN student_admission_master sam ON sam.student_id=um.student_id WHERE (um.user_password=SHA1(SHA1(MD5(CONCAT(um.pwdsalt,'" + rb.getString("current_key.first") + "','" + objesc.escapeSpecialChar(request.getParameter("pword")) + "','" + rb.getString("current_key.last") + "',um.pwdsalt)))) or um.user_password=SHA1(SHA1(MD5(CONCAT(um.pwdsalt,'" + rb.getString("next_key.first") + "','" + objesc.escapeSpecialChar(request.getParameter("pword")) + "','" + rb.getString("next_key.last") + "',um.pwdsalt))))) and STRCMP(um.user_name,'admin')=0 AND um.status>0");
                if (db.rs.next()) {
                    Admin = true;
                }
                if (Admin) {
                	...
                	String query="SELECT IFNULL(sm.department_id,sam.branch_id) db_id,um.user_id,um.roles,um.user_name,um.status,now() lastlogin,IF(um.staff_id IS NOT NULL,'Staff','Student') ss_det ,IF(um.staff_id IS NOT NULL,sm.staff_id,stu.student_id) staff_id,IF(um.staff_id IS NOT NULL,CONCAT(IFNULL(sm.first_name,''),' ',IFNULL(sm.middle_name,''),' ',IFNULL(sm.last_name,'')),CONCAT(IFNULL(stu.first_name,''),' ',IFNULL(stu.middle_name,''),' ',IFNULL(stu.last_name,''))) staff_name,IF(um.staff_id IS NOT NULL,sm.institute_email_id,stu.institute_email_id)institute_email_id FROM admin.user_master um LEFT JOIN camps.staff_master sm ON um.staff_id=sm.staff_id LEFT JOIN camps.student_master stu ON stu.student_id=um.student_id LEFT JOIN student_admission_master sam ON sam.student_id=um.student_id WHERE (um.user_password=SHA1(SHA1(MD5(CONCAT(um.pwdsalt,'" + rb.getString("current_key.first") + "','" + objesc.escapeSpecialChar(request.getParameter("pword")) + "','" + rb.getString("current_key.last") + "',um.pwdsalt)))) or um.user_password=SHA1(SHA1(MD5(CONCAT(um.pwdsalt,'" + rb.getString("next_key.first") + "','" + objesc.escapeSpecialChar(request.getParameter("pword")) + "','" + rb.getString("next_key.last") + "',um.pwdsalt))))) and STRCMP(um.user_name,'" + objesc.escapeSpecialChar(request.getParameter("uname")) + "')=0 AND um.status>0";
                	db.read(query);
                }
            }
            if (db.rs.next()) {
                if (db.rs.getRow() == 1) {
                    if (Admin) {
                        signin_status = "99";
                        ses.setAttribute("adminlogin", "true");
                    } else {
                        signin_status = "1";
                        ses.setAttribute("adminlogin", "false"); 
                    }
                    ...
                    ses.setAttribute("DMode", "off");
                    ses.setAttribute("ipaddr", ip);
                    db.read1("SELECT TRIM(MIN(startuppage)) startuppage,group_concat(role_name) role_name FROM admin.role_master WHERE role_id IN (" + db.rs.getString("roles") + ") ORDER BY startuppage DESC");
                    if (db.rs1.next()) {
            ...
            if (gkey != null) {
                db.insert("INSERT INTO admin.authentication_log (ip_address,login_time,user_name,status) VALUES('" + ip + "',NOW(),'" + gkey + "','" + signin_status + "');");
            } else {
                db.insert("INSERT INTO admin.authentication_log (ip_address,login_time,user_name,status) VALUES('" + ip + "',NOW(),'" + objesc.escapeSpecialChar(request.getParameter("uname")) + "','" + signin_status + "');");
            }
            if (ses.getAttribute("OAuth_redirect_url") != null) {
                redirect_url = ses.getAttribute("OAuth_redirect_url") + "&err=1";
            }
